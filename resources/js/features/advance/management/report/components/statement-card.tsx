...
export function StatementCard({ lines, note, report, compare }: StatementCardProps) {
    ...
    return (
        <div className="rounded-2xl border border-[var(--border-strong)] bg-[var(--card)] p-4 shadow-sm sm:p-6">
            ...
            >
                <span>{t('dashboardAdvance.report.exportColumns.description')}</span>
                <span className="text-right">
                    {compare ? t('dashboardAdvance.report.exportColumns.currentPeriod') : t('dashboardAdvance.report.exportColumns.value')}
                </span>
                {compare && <span className="hidden text-right sm:block">{t('dashboardAdvance.report.exportColumns.previousPeriod')}</span>}
                {compare && <span className="hidden text-right sm:block">Δ</span>}
            </div>

            <dl className="divide-y divide-[var(--border-strong)]">
                {lines.map((line) => (
