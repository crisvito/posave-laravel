...
export default function InventoryAdjustment({ adjustments, stats, inventoryItems, branches, is_branch_manager, filters }: InventoryAdjustmentProps) {
    ...
    return (
        ...
        >
            ...
            <div className="flex min-h-screen flex-col bg-[var(--page-bg)] p-4 sm:p-6">
                <ListToolbar
                    branch={
                        <>
                            {!is_branch_manager ? (
                    ...
                    action={
                        <CreateButton label={t('dashboardAdvance.inventoryAdjustments.list.createButton')} onClick={() => setShowCreateModal(true)} />
                    }
                />

                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-4 mb-5">
                    <div className="flex items-center gap-2 rounded-xl border border-[var(--border-strong)] bg-[var(--card)] p-3 pt-0 shadow-sm sm:gap-4 sm:p-5">
